...
void BossSkill_AquaBall::Update(Player& player)
{
    ...
    if (mIsTargetIndicating) {
        ...
        if (mAnimationTimer >= 0.05f) {
            mTargetIndicatorFrame = (mTargetIndicatorFrame + 1) % 28;
            mAnimationTimer = 0.0f;
        }
        if (mTime >= flightDuration) {
    ...
    if (mIsFalling) {
        ...
        if (t >= 1.0f) {
            ...
            mAnimationTimer += Time::DeltaTime();
            if (mAnimationTimer >= 0.05f) {
                mAquaBallFrame = (mAquaBallFrame + 1) % 25;
                mAnimationTimer = 0.0f;
            }
        }
        UpdateHitbox();
...
void BossSkill_AquaBall::Render(HDC hdc, Player& player)
{
    ...
    if (mIsTargetIndicating)
    {
        int indicatorWidth = AquaBallFallingPointAnimation[mTargetIndicatorFrame].GetWidth();
        int indicatorHeight = AquaBallFallingPointAnimation[mTargetIndicatorFrame].GetHeight();
...
{
    float scale = 2.0f;
    int imageWidth = AquaBallAnimation[0].GetWidth() * scale;
    int imageHeight = AquaBallAnimation[0].GetHeight() * scale;
